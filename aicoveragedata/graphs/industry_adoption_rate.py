...
# Average AI adoption by industry for each year.
industry_adoption_table = df.groupby(["year", "industry"])["ai_adoption_level"].mean().unstack()
years, industries, values = compare_payload(industry_adoption_table)
...
<head>
    ...
    <title>Industry Adoption Comparison</title>
    <style>
        body {{
            margin: 0;
...
<body>
    <section class="compare-card">
        <h1>Industry Adoption Comparison</h1>
        <div class="controls">
            <label>Industry 1<select id="first"></select></label>
            <label>Industry 2<select id="second"></select></label>
        </div>
    ...
    <script>
        ...
        const entities = {json.dumps(industries)};
